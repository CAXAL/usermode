...
NTSTATUS MemoryUtils::WriteMemory(uint64_t address, LPVOID dstAddress, SIZE_T nSize)
{
	if (dstAddress == 0)
		return STATUS_INVALID_PARAMETER;

	return Communication::CopyVirtualMemory(operationCallback, GetCurrentProcessId(), (uintptr_t)dstAddress, processId, address, nSize);
}
