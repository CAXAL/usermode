...
bool TraceCleaner::ExAcquireResourceExclusiveLite(PVOID Resource, BOOLEAN Wait)
{
	...
	static auto pExAcquireResourceExclusiveLite = VDM::GetKernelProcAddress(VDM::g_KernelBase, xorstr_ ("ExAcquireResourceExclusiveLite"));

	if (!pExAcquireResourceExclusiveLite)
	{
		printf(xorstr_ ("\n[IO] Error: ExAcquireResourceExclusiveLite not found.\n"));
		return 0;
	}
...
bool TraceCleaner::ExReleaseResourceLite(PVOID Resource)
{
	...
	static auto pExReleaseResourceLite = VDM::GetKernelProcAddress(VDM::g_KernelBase, xorstr_ ("ExReleaseResourceLite"));

	if (!pExReleaseResourceLite)
	{
		printf(xorstr_ ("\n[IO] Error: ExReleaseResourceLite not found.\n"));
		return false;
	}
...
BOOLEAN TraceCleaner::RtlDeleteElementGenericTableAvl(PVOID Table, PVOID Buffer)
{
	...
	static auto pRtlDeleteElementGenericTableAvl = VDM::GetKernelProcAddress(VDM::g_KernelBase, xorstr_ ("RtlDeleteElementGenericTableAvl"));

	if (!pRtlDeleteElementGenericTableAvl)
	{
		printf(xorstr_ ("\n[IO] Error: RtlDeleteElementGenericTableAvl not found.\n"));
		return false;
	}
...
uintptr_t TraceCleaner::FindPatternInKernel(uintptr_t moduleBase, uintptr_t moduleSize, LPCSTR pattern, LPCSTR mask)
{
	if (!moduleBase)
	{
		printf(xorstr_ ("\n[IO] Unable to find pattern: Invalid base address.\n"));
		return 0;
	}
...
uintptr_t TraceCleaner::FindExtendedPattern(uintptr_t moduleBase, char* sectionName, PULONG outSize)
{
	if (!moduleBase)
	{
		printf(xorstr_ ("\n[IO] Unable to find pattern in section: Invalid address.\n"));
		return 0;
	}
	...
	{
		printf(xorstr_ ("\n[IO] Unable to find pattern in section: Invalid PE.\n"));
		return 0;
	}
	...
	{
		printf(xorstr_ ("\n[IO] Failed to find pattern in section.\n"));
		return false;
	}
...
bool TraceCleaner::CleanPiDDBCacheTable()
{
	if (!VDM::g_KernelBase)
	{
		printf(xorstr_ ("\n[IO] Unable to clean PCT: Kernel base not found.\n"));
		return false;
	}

	PiDDBLockPtr = FindPatternInKernelSection(VDM::g_KernelBase, xorstr_ ("PAGE"),
		xorstr_ ("\x8B\xD8\x85\xC0\x0F\x88\x00\x00\x00\x00\x65\x48\x8B\x04\x25\x00\x00\x00\x00\x66\xFF\x88\x00\x00\x00\x00\xB2\x01\x48\x8D\x0D\x00\x00\x00\x00\xE8\x00\x00\x00\x00\x4C\x8B\x00\x24"),
			xorstr_ ("xxxxxx????xxxxx????xxx????xxxxx????x????xx?x"));

	PiDDBCacheTablePtr = FindPatternInKernelSection(VDM::g_KernelBase, xorstr_ ("PAGE"), xorstr_ ("\x66\x03\xD2\x48\x8D\x0D"), xorstr_ ("xxxxxx"));

	if (!PiDDBLockPtr)
	{
		PiDDBLockPtr = FindPatternInKernelSection(VDM::g_KernelBase, xorstr_ ("PAGE"),
			xorstr_ ("\x48\x8B\x0D\x00\x00\x00\x00\x48\x85\xC9\x0F\x85\x00\x00\x00\x00\x48\x8D\x0D\x00\x00\x00\x00\xE8\x00\x00\x00\x00\xE8"),
				xorstr_ ("xxx????xxxxx????xxx????x????x"));

		if (!PiDDBLockPtr)
		{
			printf(xorstr_ ("\n[IO] Unable to clean PCT: Lock not found.\n"));
	...
	{
		printf(xorstr_ ("\n[IO] Unable to clean PCT: Cache Table not found.\n"));
		return false;
	}
	...
	{
		printf(xorstr_ ("\n[IO] Unable to clean PCT: Cat't lock.\n"));
	...
	{
		printf(xorstr_ ("\n[IO] Unable to clean PCT: Cache not found.\n"));
	...
	{
		printf(xorstr_ ("\n[IO] Unable to clean PCT: Cat't remove from table.\n"));
...
bool TraceCleaner::CleanMmUnloadedDrivers(HANDLE hDriver)
{
	...
	{
		printf(xorstr_ ("\n[IO] Unable to clean MUD: DeviceObject not found.\n"));
	...
	{
		printf(xorstr_ ("\n[IO] Unable to clean MUD: DriverObject not found.\n"));
	...
	{
		printf(xorstr_ ("\n[IO] Unable to clean MUD: DriverSection not found.\n"));
	...
	{
		printf(xorstr_ ("\n[IO] Unable to clean MUD: Driver name not found.\n"));
...
bool TraceCleaner::CleanKernelHashBucketList(std::string driverName, std::string driverPath)
{
	std::wstring driverNameW(driverName.begin(), driverName.end());
	driverNameW = L"\\" + driverNameW;

	auto ciBase = VDM::GetKernelModule(xorstr_ ("ci.dll"));

	auto PoolBigPageTablePtr = FindPatternInKernelSection(ciBase, xorstr_ ("PAGE"), xorstr_ ("\x48\x8B\x1D\x00\x00\x00\x00\xEB\x4C\xF7\x43\x40\x00\x20\x00\x00"), xorstr_ ("xxx????xxxxx?x??"));

	if (!PoolBigPageTablePtr)
	{
		printf(xorstr_ ("\n[IO] Unable to clean KHL: KHL not found.\n"));

		return false;
	}

	auto PoolBigPageTableSizePtr = (uintptr_t)PoolBigPageTablePtr - 0x13;

	if (!PoolBigPageTableSizePtr)
	{
		printf(xorstr_ ("\n[IO] Unable to clean KHL: HCL not found.\n"));
	...
	{
		printf(xorstr_ ("\n[IO] Unable to clean KHL: HCE not found.\n"));
	...
	{
		printf(xorstr_ ("\n[IO] Unable to clean KHL: Can't lock HCL.\n"));
	...
	if (!HashEntry)
	{
		printf(xorstr_ ("\n[IO] Unable to clean KHL: KHL empty.\n"));
	...
	SIZE_T driverPathSize = (driverPath.length() - 2) * 2;

	while (HashEntry)
	{
		USHORT driverHashSize = 0;
		if (!VDM::ReadVirtualMemory((uintptr_t)HashEntry + offsetof(HASH_BUCKET_ENTRY, DriverName.Length), &driverHashSize, sizeof(driverHashSize)) || driverHashSize == 0)
		{
			ExReleaseResourceLite(g_HashCacheLock);
		...
		if (driverPathSize == driverHashSize)
		{
			wchar_t* driverHashBuffer = 0;
			if (!VDM::ReadVirtualMemory((uintptr_t)HashEntry + offsetof(HASH_BUCKET_ENTRY, DriverName.Buffer), &driverHashBuffer, sizeof(driverHashBuffer)) || !driverHashBuffer)
			{
				ExReleaseResourceLite(g_HashCacheLock);
			...
			wchar_t* driverHashName = new wchar_t[(ULONG64)driverHashSize / 2ULL + 1ULL];
			memset(driverHashName, 0, driverHashSize + sizeof(wchar_t));

			if (!VDM::ReadVirtualMemory((uintptr_t)driverHashBuffer, driverHashName, driverHashSize))
			{
				ExReleaseResourceLite(g_HashCacheLock);
			...
			{
				HASH_BUCKET_ENTRY* NextEntry = 0;

				if (!VDM::ReadVirtualMemory((uintptr_t)HashEntry, &NextEntry, sizeof(NextEntry)))
				{
					ExReleaseResourceLite(g_HashCacheLock);
				...
				if (!VDM::WriteVirtualMemory((uintptr_t)PrevHashEntry, &NextEntry, sizeof(NextEntry)))
				{
					ExReleaseResourceLite(g_HashCacheLock);
				...
				if (!VDM::ExFreePool((uintptr_t)HashEntry))
				{
					ExReleaseResourceLite(g_HashCacheLock);
		...
		if (!VDM::ReadVirtualMemory((uintptr_t)HashEntry, &HashEntry, sizeof(HashEntry)))
		{
			ExReleaseResourceLite(g_HashCacheLock);
