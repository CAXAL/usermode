...
{
	static const std::string registryKey = xorstr_("System\\CurrentControlSet\\Services\\");
...
{
	std::string registryKey = xorstr_("System\\CurrentControlSet\\Services\\") + serviceName;
	...
	status = RegSetValueExA(hKey, xorstr_("ImagePath"), NULL, REG_SZ, (std::uint8_t*)driverPath.c_str(), static_cast<DWORD>(driverPath.size()));
	...
	status = RegSetValueExA(hKey, xorstr_("Type"), NULL, REG_DWORD, &typeValue, 4u);
...
{
	if (!GrantPrivileges(xorstr_(L"SeLoadDriverPrivilege")))
	{
		printf(xorstr_("\n[IO] Failed to set privilege.\n"));
		return false;
	}

	if (!CreateServiceEntry("\\??\\" + std::filesystem::absolute(std::filesystem::path(driverPath)).string(), serviceName))
	{
		printf(xorstr_("\n[IO] Failed to create service.\n"));
		return false;
	}

	std::string registryPath = xorstr_("\\Registry\\Machine\\System\\CurrentControlSet\\Services\\") + serviceName;
