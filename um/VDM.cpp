...
{
	const std::string ALPHA_BET = xorstr_ ("qwertyuiopasdfghjklzxcvbnmQWERTYUIOPASDFGHJKLZXCVBNM");
...
{
	hDriver = hDevice;
	g_KernelBase = GetKernelModule(xorstr_ ("ntoskrnl.exe"));
...
bool VDM::IsLoaded()
{
	auto deviceName = xorstr_("\\\\.\\Global\\EneIo");

	hDriver = CreateFileA(deviceName, FILE_ANY_ACCESS, 0, nullptr, OPEN_EXISTING, FILE_ATTRIBUTE_NORMAL, nullptr);
...
std::pair<HANDLE, std::string> VDM::Load(std::vector<std::uint8_t>& driverBuffer, std::string& outPath)
{
	...
	HANDLE hDriver = CreateFileA(deviceName, GENERIC_READ | GENERIC_WRITE, 0, nullptr, OPEN_EXISTING, FILE_ATTRIBUTE_NORMAL, NULL);
...
{
	static auto pMemcpy = GetKernelProcAddress(g_KernelBase, xorstr_ ("memcpy"));

	if (!pMemcpy)
	{
		printf(xorstr_ ("\n[IO] Error: memcpy not found.\n"));
...
bool VDM::SetMemory(uint64_t address, uint32_t value, uint64_t size)
{
	...
	static auto pMemset = GetKernelProcAddress(g_KernelBase, xorstr_ ("memset"));

	if (!pMemset)
	{
		printf(xorstr_ ("\n[IO] Error: memset not found.\n"));
...
uint64_t VDM::ExAllocatePool(ntspace::POOL_TYPE pool_type, uint64_t size)
{
	...
	static auto pExAllocatePool = GetKernelProcAddress(g_KernelBase, xorstr_ ("ExAllocatePoolWithTag"));

	if (!pExAllocatePool)
	{
		printf(xorstr_ ("\n[IO] Error: ExAllocatePool not found.\n"));
...
bool VDM::ExFreePool(uint64_t address)
{
	...
	static auto pExFreePool = GetKernelProcAddress(g_KernelBase, xorstr_ ("ExFreePool"));

	if (!pExFreePool)
	{
		printf(xorstr_ ("\n[IO] Error: ExFreePool not found.\n"));
...
{
	static auto pMmAllocatePagesForMdl = GetKernelProcAddress(g_KernelBase, xorstr_ ("MmAllocatePagesForMdl"));
...
{
	static auto pMmMapLockedPagesSpecifyCache = GetKernelProcAddress(g_KernelBase, xorstr_ ("MmMapLockedPagesSpecifyCache"));
...
{
	static auto pMmProtectMdlSystemAddress = GetKernelProcAddress(g_KernelBase, xorstr_ ("MmProtectMdlSystemAddress"));
...
{
	static auto pMmUnmapLockedPages = GetKernelProcAddress(g_KernelBase, xorstr_ ("MmUnmapLockedPages"));
...
{
	static auto pMmFreePagesFromMdl = GetKernelProcAddress(g_KernelBase, xorstr_ ("MmFreePagesFromMdl"));
