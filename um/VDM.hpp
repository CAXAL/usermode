...
public:
	...
	static std::pair<HANDLE, std::string> Load(std::vector<std::uint8_t>& driverBuffer, std::string& outPath);
	...
	static bool SystemCall(uint64_t address, ReturnType* result, const ArgPack ...args)
	{
		...
		if (!UserSyscall)
		{
			printf("\n[IO] Error: User syscall not found.\n");
			return false;
		}
		...
		if (!KernelSyscall)
		{
			printf("\n[IO] Error: Kernel syscall not found.\n");
			return false;
		}
