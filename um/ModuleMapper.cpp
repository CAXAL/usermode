...
uint64_t GetFuncAddress(const char* moduleName, const char* funcName)
{
	...
	if (!remoteBase)
	{
		printf(xorstr_ ("\n[IO] Module base for %s not found.%ws\n"), moduleNameUTF.c_str());
		Exit;
	}
...
VOID MapSections(PIMAGE_NT_HEADERS pNtHeader, PVOID moduleBase, PVOID localBase)
{
		...
		pSectionHeader < pFirstSection + pNtHeader->FileHeader.NumberOfSections; pSectionHeader++)
	{
		...
		ModuleMapper::memoryInstance->WriteMemory((uint64_t)((uintptr_t)moduleBase + pSectionHeader->VirtualAddress), (LPVOID)((uintptr_t)localBase + pSectionHeader->PointerToRawData), pSectionHeader->SizeOfRawData);
	}
}
...
VOID EraseDiscardableSection(PIMAGE_NT_HEADERS pNtHeader, PVOID moduleBase)
{
		...
		pSectionHeader < pFirstSection + pNtHeader->FileHeader.NumberOfSections; pSectionHeader++)
	{
		...
		if (pSectionHeader->Characteristics & IMAGE_SCN_MEM_DISCARDABLE || !strcmp((const char*)pSectionHeader->Name, ".rsrc"))
		{
			...
			ModuleMapper::memoryInstance->WriteMemory((uint64_t)((uintptr_t)moduleBase + pSectionHeader->VirtualAddress), (LPVOID)filledData, pSectionHeader->SizeOfRawData);
...
VOID ErasePE(PIMAGE_NT_HEADERS pNtHeader, PVOID moduleBase)
{
	...
	memoryUtils->WriteMemory((uint64_t)((uintptr_t)moduleBase), (LPVOID)filledData, pNtHeader->OptionalHeader.SizeOfHeaders);
...
{
	uint64_t User32Base = memoryUtils->GetModuleBase(xorstr_ (L"user32.dll"));

	if (!User32Base)
	{
		printf(xorstr_ ("\n[IO] Failed to get base."));
		Exit;
	}

	if (!ParseImports(User32Base))
	{
		printf(xorstr_ ("\n[IO] Failed to parse imports."));
		Exit;
	}

	uint64_t TargetImportFunction = imports[xorstr_ ("NtUserGetForegroundWindow")];

	if (!TargetImportFunction)
	{
		printf(xorstr_ ("\n[IO] Target import not found."));
		Exit;
	}
	...
	*reinterpret_cast<uint64_t*>(&ImportTableWorkerCode[46]) = DllMainFunc;
	...
	if (!ImportWorkerCode)
	{
		printf(xorstr_ ("\n[IO] Failed to allocate pe worker."));
		Exit;
	}

	memoryUtils->WriteMemory(ImportWorkerCode, (LPVOID)ImportTableWorkerCode, sizeof(ImportTableWorkerCode));
	...
	memoryUtils->WriteMemory(TargetImportFunction, (LPVOID)&InjectedStatus, sizeof(InjectedStatus));
	...
	for (BYTE EntryStatus = 0;; Sleep(10))
	{
		...
		if (!NT_SUCCESS(Status))
		{
			printf(xorstr_ ("\n[IO] Failed to read entry status."));
			Exit;
		}
	...
	if (!NT_SUCCESS(Status))
	{
		printf(xorstr_ ("\n[IO] Failed to free memory. Error: %p\n"), Status);
		Exit;
	}
}
...
BOOL ModuleMapper::LoadModule(PVOID LocalImage, bool Delay)
{
	...
	memoryUtils->WriteMemory(moduleBase, (LPVOID)LocalImage, pImageNtHeader->OptionalHeader.SizeOfHeaders);
